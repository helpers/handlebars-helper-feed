...
module.exports.register = function (Handlebars, options, params) {
  ...
  // The {{feed}} helper
  Handlebars.registerHelper('feed', function(context, options) {
    options = _.extend(context, options);
    var hash = options.hash || {};

    // Default options
    options = _.extend({glob: {}, sep: '\n'}, options, opts.feed, hash);
    var i = 0;
    var result = '';
    var data;
    ...
    patterns.forEach(function (pattern) {
      ...
      var src = files.map(function (filepath) {
        ...
        data = _.extend(data, opts.feed, _.omit(metadata, ['assemble', 'pages', 'plugins', '_plugins']));
        ...
        // Format date to RFC-822 datetime for XML feed
        data.date   = moment(datetime).format('ddd, DD MMM YYYY HH:mm:ss ZZ');
